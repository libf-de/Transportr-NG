...
//    ): List<GenericLocation>
    ...
    suspend fun addFavoriteLocation(location: FavoriteLocation): Long
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun addHomeLocation(location: HomeLocation): Long
    ...
    suspend fun countHomes(networkId: NetworkId?): Int

    // WorkLocation
    @Query("SELECT * FROM work_locations WHERE networkId = :networkId")
    fun getWorkLocationAsFlow(networkId: NetworkId?): Flow<WorkLocation?>

    //	@Query("SELECT * FROM work_locations WHERE networkId = :networkId")
    //	LiveData<WorkLocation> getWorkLocation(NetworkId networkId);
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun addWorkLocation(location: WorkLocation): Long

    /* This is just for tests to ensure, there's only ever one home location per network */
    @Query("SELECT COUNT(uid) FROM work_locations WHERE networkId = :networkId")
    suspend fun countWorks(networkId: NetworkId?): Int
}
